...
export default function DoctorProfile() {
  ...
  const [requestSent, setRequestSent] = useState(false) // ✅ état demande envoyée
  ...
  const handleBook = async (e) => {
    e.preventDefault()
    if (!bookingDate || !bookingTime) { setError("Veuillez sélectionner la date et l'heure."); return }
    setSubmitting(true)
    setError('')
    setMessage('')
    try {
      ...
      setMessage('✅ Demande de rendez-vous envoyée ! Le médecin examinera votre demande.')
      setBookingDate('')
      setBookingTime('')
    } catch (err) {
  ...
  // ✅ MODIFIÉ : envoie une demande (pending) au lieu d'ouvrir le chat directement
  const handleRequestConsultation = async () => {
    setSubmitting(true)
    setError('')
    try {
      await axios.post(`${API}/consultations/request`, {
        doctor_id: id,
        type: 'online'
      }, { headers })
      setRequestSent(true) // Afficher le message d'attente
    } catch (err) {
      setError(err.response?.data?.message || 'Impossible d\'envoyer la demande. Veuillez réessayer.')
    } finally {
      setSubmitting(false)
    }
  }
  ...
  if (error && !doctor) {
    return (
      <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center p-6">
        <div className="bg-red-50 text-red-600 px-6 py-4 rounded-xl border border-red-200 mb-4 max-w-md text-center">{error}</div>
        <button onClick={() => navigate('/home')} className="text-green-600 font-semibold hover:underline">Retour à l'accueil</button>
      </div>
    )
  }
    ...
    <div className="min-h-screen bg-gray-50 font-sans pb-12">
      <header className="bg-white border-b px-6 py-4 sticky top-0 z-40 shadow-sm flex items-center justify-between">
        <div className="flex items-center gap-3">
          <span onClick={() => navigate('/home')} className="text-2xl font-extrabold text-green-600 cursor-pointer">MediCam</span>
          <span className="text-sm font-medium text-gray-400">|</span>
          <span className="text-sm font-medium text-gray-600">Profil Médecin</span>
        </div>
        <button onClick={() => navigate(-1)} className="text-sm text-gray-600 hover:text-green-600 font-semibold transition-colors">← Retour</button>
      </header>
      ...
      <main className="max-w-4xl mx-auto px-4 mt-8 grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="md:col-span-2 space-y-6">
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 flex flex-col sm:flex-row gap-6 items-center sm:items-start">
            <div className="w-24 h-24 rounded-full bg-green-50 border-2 border-green-200 flex items-center justify-center text-4xl shadow-inner flex-shrink-0">👨‍⚕️</div>
            <div className="flex-1 text-center sm:text-left space-y-2">
              <h2 className="text-2xl font-bold text-gray-800">{formatDoctorName(doctor.full_name)}</h2>
          ...
          {/* ✅ Bloc consultation immédiate — avec salle d'attente */}
          <div className="bg-gradient-to-br from-green-50 to-emerald-50 border border-green-200 rounded-2xl p-6 shadow-sm">
            {requestSent ? (
              // ✅ Message d'attente après demande envoyée
              <div className="flex flex-col items-center text-center gap-3">
                <div className="w-12 h-12 rounded-full bg-green-100 flex items-center justify-center text-2xl">⏳</div>
                <div>
                  <h3 className="font-bold text-green-800 text-lg">Demande envoyée !</h3>
                  <p className="text-sm text-green-700 mt-1">
                    Votre demande est en attente de confirmation du médecin.<br/>
                    Vous serez notifié dès qu'il l'accepte. La demande expire après 30 minutes.
                  </p>
                </div>
                <button
                  onClick={() => navigate('/consultations')}
                  className="mt-2 text-sm text-green-700 underline font-medium"
                >
              ...
              <div className="flex flex-col md:flex-row justify-between items-center gap-4">
                <div className="space-y-1">
                  ...
                  <p className="text-sm text-green-700">Envoyez une demande au médecin. Il doit l'accepter avant d'ouvrir le chat.</p>
                </div>
                <button
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 h-fit space-y-4">
          <h3 className="font-bold text-gray-800 text-lg">Demander un rendez-vous</h3>
          {message && (
            <div className="bg-green-50 border border-green-200 text-green-600 text-xs rounded-lg p-3">{message}</div>
          )}
          <form onSubmit={handleBook} className="space-y-3">
            ...
            <label className="block">
              <span className="text-xs text-gray-500 font-semibold uppercase">Heure souhaitée</span>
            ...
            <label className="block">
              ...
              <select required value={bookingType} onChange={(e) => setBookingType(e.target.value)}
                className="mt-1 w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-green-500">
            ...
            <label className="block">
              ...
              <select required value={bookingDuration} onChange={(e) => setBookingDuration(Number(e.target.value))}
                className="mt-1 w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-green-500">
